...
class IA2C(IndependentAgent):
    def __init__(self, config, obs_act, map_name, thread_number):
        super().__init__(config, obs_act, map_name, thread_number)
        for key in obs_act:
            obs_space = obs_act[key][0]
            act_space = obs_act[key][1]
            self.agents[key] = PFRLA2CAgent(config, obs_space, act_space)
            if self.config['load']:
...
class PFRLA2CAgent(Agent):
    def __init__(self, config, obs_space, act_space):
        ...
        self.agent = A2C(self.model, self.optimizer, gpu=self.device.index, num_processes = 8,
